...
public class CreateSalonOwnerUseCaseTests
{
    ...
    public CreateSalonOwnerUseCaseTests()
    {
        ...
        _inputNormalizerMock
            ...
            .Returns<string>(value => string.Join(" ", value.Trim().Split(' ', StringSplitOptions.RemoveEmptyEntries)));
        _inputNormalizerMock
            .Setup(normalizer => normalizer.NormalizeDocument(It.IsAny<string>()))
            .Returns<string>(value => new string(value.Where(char.IsDigit).ToArray()));
        _unitOfWorkMock
    ...
    [Fact]
    public async Task Should_Normalize_Document_Before_CheckingUniqueness()
    {
        var request = new CreateSalonOwnerRequestDto(
            "Maria Owner",
            "maria.owner@email.com",
        ...
        _salonRepositoryMock.Verify(repository => repository.ExistsBySalonDocumentAsync("123456789"), Times.Once);
    }
}
